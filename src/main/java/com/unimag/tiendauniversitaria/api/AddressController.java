...
public class AddressController {
    ...
    @PostMapping
    public ResponseEntity<AddressDtos.AddressResponse> create(@Valid @RequestBody AddressDtos.AddressCreateRequest req,
                                                              UriComponentsBuilder uriBuilder) {
        var body = service.create(req);
        var location = uriBuilder.path("/api/v1/addresses/{id}")
                .buildAndExpand(body.id()).toUri();
        return ResponseEntity.created(location).body(body);
    }

    @GetMapping("/{id}")
    public ResponseEntity<AddressDtos.AddressResponse> get(@PathVariable Long id) {
        return ResponseEntity.ok(service.get(id));
    }

    @GetMapping
    public ResponseEntity<Page<AddressDtos.AddressResponse>> list(@RequestParam(defaultValue = "0") int page,
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<AddressDtos.AddressResponse> update(@PathVariable Long id,
                                                              @Valid @RequestBody AddressDtos.AddressUpdateRequest req) {
